...
def convert_docx_to_markdown(docx_path, output_dir, create_subfolder=True, output_name=None,
    ...
    if create_subfolder:
        ...
        # folder_name 已做词法清洗，但输出根目录下的同名符号链接仍可把
        # 写入与 prune_stale_assets 的删除重定向到 output_dir 之外，
        # 必须在创建目录前拒绝，并校验解析后的真实路径未逃逸。
        if os.path.islink(final_output_dir):
            raise ValueError(f"输出子目录不允许为符号链接: {final_output_dir}")
        output_root_real = os.path.realpath(output_dir)
        final_real = os.path.realpath(final_output_dir)
        if final_real != output_root_real and not final_real.startswith(
            output_root_real + os.sep
        ):
            raise ValueError(
                f"输出目录解析后逃逸出 output_dir: {final_output_dir} -> {final_real}")
    else:
