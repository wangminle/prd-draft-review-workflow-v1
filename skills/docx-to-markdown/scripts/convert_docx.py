    ...
    with zipfile.ZipFile(docx_path, 'r') as zip_ref:
        ...
        for file_info in zip_ref.filelist:
            if file_info.filename.startswith('word/media/'):
                # 本仓语义：图片数量超限停止提取（模块常量可被测试 patch）
...
def convert_docx_to_markdown(docx_path, output_dir, create_subfolder=True, output_name=None):
    ...
    try:
        ...
        # 本仓语义：入口只做 zip bomb/条目/总量防线；超大媒体在提取阶段
        # 降级跳过而非整篇拒绝（与审计契约一致），故不调用
        # validate_docx_zip_security 的媒体大小硬拒绝分支。
    ...
    # 获取文件名（不含扩展名）；output_name 优先于 docx_path 派生名
    if output_name:
...
# ── 本仓库（prd-draft-review-workflow-v1）兼容层 ──────────────────────
# 上游 V0.1.6 将旧符号重构为 DOCX_SECURITY_LIMITS/sha256_file 等；为避免
# 集成点（docx_conversion_worker、审计测试）随上游改名联动修改，保留旧名称
# 作为稳定接口。

# 旧资源限制常量：值与 DOCX_SECURITY_LIMITS 一致；模块级独立变量使
# mock.patch.object(module, "MAX_...") 可生效（审计测试依赖此能力）。
MAX_TOTAL_UNCOMPRESSED = DOCX_SECURITY_LIMITS["total_uncompressed"]
MAX_SINGLE_ENTRY_UNCOMPRESSED = DOCX_SECURITY_LIMITS["entry_uncompressed"]
MAX_COMPRESSION_RATIO = DOCX_SECURITY_LIMITS["entry_ratio"]
MAX_TOTAL_COMPRESSION_RATIO = DOCX_SECURITY_LIMITS["total_ratio"]
MIN_COMPRESSED_FOR_TOTAL_RATIO_CHECK = DOCX_SECURITY_LIMITS["total_ratio_min_compressed"]
MAX_IMAGE_COUNT = DOCX_SECURITY_LIMITS["image_count"]
MAX_SINGLE_IMAGE_SIZE = DOCX_SECURITY_LIMITS["image_file_size"]
MAX_IMAGE_PIXELS = DOCX_SECURITY_LIMITS["image_pixels"]
MAX_XLSX_SIZE = DOCX_SECURITY_LIMITS["embedded_excel_size"]


def validate_zip_safety(docx_path: str) -> None:
    """基于路径的整体 ZIP 安全校验（读模块级 MAX_* 常量，可被测试 patch）。

    与 validate_docx_zip_security 的差异：入口接收文件路径并在内部打开，
    且直接引用上方模块级常量而非 DOCX_SECURITY_LIMITS 字典。
    """
    total_compressed = 0
    total_uncompressed = 0
    with zipfile.ZipFile(docx_path, "r") as zf:
...
def _get_image_pixel_count(image_data: bytes) -> Optional[int]:
    ...
    try:
        ...
        pass
    return None
    # 注意：本实现为宽松头解析（PNG 仅需前 24 字节，不校验 IHDR chunk 标记），
    # 与上游 image_pixel_count（严格 chunk 结构）并存；本仓降级判定与审计
    # 契约绑定此行为，故保留旧实现而非委托上游。



def _image_resource_rejection_reason(image_data: bytes) -> Optional[str]:
    """返回图片资源拒绝原因（单图大小/像素）；符合限制时返回 None。

    本仓降级语义依赖此判定决定“跳过该资源”而非“拒绝整篇文档”。
    """
...
# 旧哈希/sentinel 接口别名（sha256_file 与 write/read_conversion_sentinel 为上游新名）
compute_file_sha256 = sha256_file


def write_converted_sentinel(output_dir: str, folder_name: str, docx_path: str) -> None:
    """旧签名：由 docx_path 现算源哈希后写 .converted sentinel。"""
    write_conversion_sentinel(output_dir, folder_name, sha256_file(docx_path))


def read_sentinel_source_sha256(sentinel_path: str) -> Optional[str]:
    """旧签名：接收 sentinel 文件路径（而非目录），返回源 SHA-256 或 None。

    旧格式（纯文本 folder_name）、损坏 JSON 或缺哈希字段一律返回 None。
    """
    try:
        with open(sentinel_path, "r", encoding="utf-8") as f:
            payload = json.load(f)
