...
面向团队协作的需求评审工作流平台，重点解决 PRD 从上传、拆解、逐篇分析、系统评审到报告生成的全流程闭环问题。项目采用内网可部署架构，强调可追溯、可配置、可扩展，以及运行时数据与源码分离。当前版本 V0.2.11。

### 架构设计

系统当前采用四层协作结构：

- 接入层：FastAPI 提供认证、对话、上传、历史、管理、需求评审、团队空间等 API，原生 JavaScript SPA 提供统一前端界面。
...
src/app/routers/            API 路由层（auth/chat/upload/history/admin/review/workspace）
src/app/services/           应用服务、SkillRunner、LLM 适配、品牌配置
src/app/repositories/       数据访问层（含 Workspace/KnowledgeSource/ProjectSourceRef）
...
- Access layer: FastAPI exposes authentication, chat, upload, history, admin, review, and workspace APIs, while a vanilla JavaScript SPA provides the unified UI.
...
src/app/routers/            API routes (auth/chat/upload/history/admin/review/workspace)
src/app/services/           Application services, SkillRunner, LLM integration, branding config
src/app/repositories/       Persistence layer (incl. Workspace/KnowledgeSource/ProjectSourceRef)
