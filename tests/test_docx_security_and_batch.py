...
class TestOutputSubfolderSymlinkGuard(unittest.TestCase):
    """输出子目录符号链接防线：防止写入与 prune_stale_assets 沿链接删除 output_dir 之外的文件。"""

    @classmethod
    def setUpClass(cls):
        cls.convert = load_module("convert_docx_symlink_guard", SCRIPTS_DIR / "convert_docx.py")

    def test_output_subfolder_symlink_rejected_and_target_untouched(self):
        with tempfile.TemporaryDirectory() as tmp:
            docx_path = os.path.join(tmp, "input.docx")
            build_minimal_docx(docx_path)

            victim_dir = os.path.join(tmp, "victim")
            victim_assets = os.path.join(victim_dir, "assets")
            os.makedirs(victim_assets)
            keep = os.path.join(victim_assets, "keep.bin")
            with open(keep, "wb") as f:
                f.write(b"precious")

            output_root = os.path.join(tmp, "out")
            os.makedirs(output_root)
            # input.docx 的 folder_name 为 input；out/input 指向 victim，
            # 使 assets 目录本身是普通目录、只靠父级链接越界
            os.symlink(victim_dir, os.path.join(output_root, "input"))

            with self.assertRaises(ValueError) as ctx:
                self.convert.convert_docx_to_markdown(docx_path, output_root)
            self.assertIn("符号链接", str(ctx.exception))
            # 链接目标完全未被触碰：无删除、无新产物写入
            self.assertTrue(os.path.exists(keep))
            self.assertEqual(os.listdir(victim_assets), ["keep.bin"])
            self.assertEqual(os.listdir(victim_dir), ["assets"])
