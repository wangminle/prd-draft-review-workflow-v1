...
@pytest.mark.asyncio
async def test_default_admin_uses_preset_password_on_init(tmp_path, monkeypatch, caplog):
...
@pytest.mark.asyncio
async def test_preset_password_is_warned_not_reset(tmp_path, monkeypatch, caplog):
    import app.database as database_module
    ...
    assert any("默认预设口令" in record.message or "弱口令" in record.message for record in caplog.records)
    assert admin.password_hash == preset_hash
    assert verify_password(DEFAULT_ADMIN_PASSWORD, admin.password_hash) is True
...
@pytest.mark.asyncio
async def test_legacy_admin123_is_still_warned(tmp_path, monkeypatch, caplog):
    import app.database as database_module
