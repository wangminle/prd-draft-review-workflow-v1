...
> 版本号对应 `src/main.py` 中的 `APP_VERSION`（当前为 `0.3.8`）。以下端点均来自源码 `src/main.py` 与 `src/app/routers/`,如发现与代码不一致,以代码为准。
...
| 方法 | 路径 | 用途 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/api/health` | 健康检查,返回 `{"status":"ok","version":"0.3.8"}` | 公开 |
...
| --- | --- | --- | --- |
| GET | `/api/health` | Health check, returns `{"status":"ok","version":"0.3.8"}` | Public |
