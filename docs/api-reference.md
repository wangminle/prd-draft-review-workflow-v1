...
> 版本号对应根目录 `VERSION` 文件（当前为 `0.3.15`，运行时由 `src/app/version.py` 读取为 `APP_VERSION`）。以下端点均来自源码 `src/main.py` 与 `src/app/routers/`，如发现与代码不一致，以代码为准。
...
| 方法 | 路径 | 用途 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/api/health` | 健康检查,返回 `{"status":"ok","version":"0.3.15"}` | 公开 |
...
| --- | --- | --- | --- |
| GET | `/api/health` | Health check, returns `{"status":"ok","version":"0.3.15"}` | Public |
