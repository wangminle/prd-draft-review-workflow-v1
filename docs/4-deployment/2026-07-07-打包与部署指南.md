...
> 适用版本：V0.3.4（2026-07-09）。本文讲「代码如何从开发机分发到目标服务器」，与 [Ubuntu + Nginx + systemd 部署与安全加固方案](2026-05-22-ubuntu-nginx-systemd-security-plan.md) 互补——那篇讲目标服务器的网络拓扑、守护进程与安全加固，本文讲打包产物与分发安装流程。
...
#    默认 zip 分发包：
unzip prd-draft-review-workflow-v1-code-config-v0.3.4-build*.zip -d /opt/ai-review
#    若是 tar.gz 分发包：tar -xzf prd-draft-review-workflow-v1-code-config-v0.3.4-build*.tar.gz -C /opt/ai-review
cd /opt/ai-review

# 2) 配置密钥（包内只有 .env.example，不含真实密钥）
cp .env.example .env
#    编辑 .env：填 DEEPSEEK_API_KEY/QWEN_API_KEY/GLM_API_KEY/OPENAI_API_KEY；
#              JWT_SECRET 改为随机串（可用 python3 -c "import secrets;print(secrets.token_hex(32))"）；
...
健康检查端点：`GET http://<host>:<port>/api/health`，正常返回 `{"status":"ok","version":"0.3.4"}`。
...
./update.sh
# 或显式指定包路径
./update.sh --package ./prd-draft-review-workflow-v1-code-config-v0.3.4-build202607071605.tar.gz
```
...
| 启动后健康检查失败 | 看 `runtime/logs/app.log`；常见为 `.env` 缺密钥、端口被占、依赖未装全 |
...
- [ ] `.env` 中至少一个 LLM API Key 可用，`JWT_SECRET` 已改为随机串
- [ ] 浏览器访问 `http://<host>:<port>` 能打开登录页
- [ ] 首个管理员账号已初始化（生产环境建议关闭 `allow_public_registration`）
