...
2. **P2/P3**：补轻量连接器、LanceDB 单引擎检索（POC-A+B 已验证）、拒答策略（分数差+LLM 证据判断）、`AgentRun / ToolCallTrace`、`WorkflowTemplate` 内置模板。
...
8. **成员管理交互**（P1 补充）：角色变更用 inline dropdown + 确认弹窗，选项为 owner→admin→member→viewer（只能降不能升自身角色）；停用/恢复用 badge 状态切换 + 确认弹窗，停用后成员行灰化（`opacity: 0.5`）+ 角色标签旁显示"已停用"；权限提示用 tooltip 或行内灰字说明"仅 owner/admin 可管理"；管理操作只对 owner/admin 可见，不能变更自身角色，owner 角色变更需二次确认（防止最后一个 owner 误降级）。
...
| 全文检索 | SQLite FTS5 | LanceDB 不可用时降级回退 | 第一版低成本、可控、易测试 |
| 向量库 | LanceDB 单引擎（POC-A+B ✅ 已验证，详见 `docs/3-design/检索引擎选型最终结论.md`） | 数据规模扩大后评估 Milvus Standalone/Distributed、Qdrant 等服务化方案 | 近期优先低运维和 `runtime/` 友好，中后期再服务化 |
| Embedding | OpenAI text-embedding-3-small（POC-A+B 结论） | 内网模型成熟后可替换 BGE-M3 | API 嵌入成本低、向量已归一化，避免 TF-IDF fallback 负分问题 |
...
### 8.2 向量库选型（已确定，POC-A+B ✅ 已完成）

> 选型已通过 POC-A（5 方案横向对比）和 POC-B（混合检索深度验证）正式确定。
> 最终结论详见 `docs/3-design/检索引擎选型最终结论.md`。

P2 首版选型：**LanceDB 单引擎**（95.2% top-5 命中率、P50=10ms、120 QPS、100% 权限过滤、7ms 备份）。
FTS5 作为降级回退（LanceDB 不可用时），拒答策略采用 top-1/top-2 分数差 + LLM 证据判断。

POC-A+B 关键发现：
1. Hybrid 混合检索反而比纯 LanceDB 低 2.3%（92.9% < 95.2%），首版不引入 Hybrid
2. 原始分数阈值拒答方案完全失败（误答率 25%-100%），需用分数差替代
3. TF-IDF fallback 向量未归一化导致负分问题，P2 使用 OpenAI embedding 替代

| 方案 | 选型定位 | POC-A+B 数据 | 本项目判断 |
| --- | --- | --- | --- |
| SQLite FTS5 | ✅ 降级回退 | top-5 42.9%, P50=0.2ms | LanceDB 不可用时回退，关键词兜底 |
| LanceDB | ✅ **P2 首选** | top-5 95.2%, P50=10ms, 120 QPS | 质量达标、延迟低、部署简单、runtime 隔离 |
| Chroma | 🔄 备选 | top-5 95.2%, P50=4.8ms | 需简化依赖栈时评估 |
| sqlite-vec | 🔄 轻量备选 | top-5 95.7%, P50=6.5ms | 去掉 pyarrow 时评估，维度限制 8192 |
| Milvus Lite | 🔄 远期候选 | top-5 95.2%, P50=530ms | 服务化部署时迁移到 Milvus Standalone |
| Milvus Standalone / Distributed | 远期 | 未测 | 数据规模扩大后重点评估 |
| Qdrant | 远期备选 | 未测 | 与 Milvus 服务化方案一起评估 |

建议：**P2 首版用 LanceDB 单引擎做语义检索（95.2% top-5 命中率），FTS5 作为 LanceDB 不可用时的降级回退；拒答策略采用 top-1/top-2 分数差 + LLM 证据判断；嵌入模型采用 OpenAI text-embedding-3-small；资料规模扩大后再评估 Milvus Standalone/Distributed、Qdrant 等服务化向量库。**

### 8.3 嵌入模型选型（P2 已确定：OpenAI text-embedding-3-small）

> POC-A/B 均使用 TF-IDF fallback，发现向量未归一化导致 Hybrid score normalization 失真。
> P2 正式使用 OpenAI text-embedding-3-small，向量已归一化，cosine distance ∈ [0,2]，不会出现负分问题。

P2 首版选型：**OpenAI text-embedding-3-small**（1536 维，$0.02/1M tokens，归一化向量）。
备选：BGE-M3 本地部署（1024 维，免费但需算力）。
